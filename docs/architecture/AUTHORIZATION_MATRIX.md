...
| **Golden Key Staff** | Human | Single org | Coaches/advisors. Operate the CRM, roadmaps, documents, reports, and reviews within their organization only. |
...
Columns: **PA** = Platform Admin, **OO** = Organization Owner, **ST** = Golden Key Staff, **CL** = Client, **PV** = Partner Viewer (later), **WK** = Worker service, **AI** = AI orchestration service.

| Resource family | PA ᵃ | OO | ST | CL ᶜ | PV ᵈ | WK ᵉ | AI ᶠ |
|---|---|---|---|---|---|---|---|
| Organizations | CRUD | RU | R | – | – | R | – |
| Memberships & invitations | CRUD | CRUD ᵇ | R | – | – | R | – |
| Leads | R | CRUD | CRUD | – | – | R | R |
| Clients (CRM record) | R | CRUD | CRU | R (own) | – | R | R |
| Notes & communications | R | CRUD | CRU | – | – | CR ᵍ | R ᵖ |
| Financial facts | R | CRU | CRU | CRU (own) ʰ | – | R | R (approved facts only) |
| Goals | R | CRU | CRU | CRU (own) | – | R | R |
| Readiness assessments | R | R / A | R / A | R (own) | – | C ⁱ | R |
| Rule versions | CRUD | R | R | – | – | R | R |
| Roadmaps, milestones, actions, tasks | R | CRUD / A | CRU / A | R (own); U tasks ʲ | – | R | R |
| Education catalog | CRUD | CR | R | – | – | R | R |
| Education assignments | R | CRU | CRU | R (own) | – | CR ᵍ | – |
| Documents | R | CRUD / A | CRU / A | CR (own) ᵏ | – | RU ˡ | R (metadata + review status) |
| Appointments & reminders | R | CRUD | CRUD | CRU (own) | – | CR ᵍ | R |
| Quarterly reports | R | R / A / E | R / A / E | R (approved, own) | – | C ⁱ | R |
| Partner directory | R | CRUD ᵇ | R | – | – | R | R |
| Referrals | R | CRU / A | CRU | R (own) | R (shared) | R | R |
| Engagement analytics | R | R | R | – | – | CR ⁱ | R |
| Round-up simulator | R | R | R | CRU (own) | – | C ⁱ | R |
| AI runs & recommendations | R | R / A | R / A | R (own, approved) ᵐ | – | C ⁱ | C ⁿ |
| Audit events | R / E | R (own org) | – | – | – | C | – |
| Consent & data-sharing grants | R | R | R | CRU (own) ᵒ | – | R | R |
...
- **b — Owner-only management.** Only the Organization Owner manages memberships (invite, role change, removal) and the partner directory (add/edit/deactivate partners, referral rules). Staff can read both but change neither.
...
- **m — Clients see AI-derived content only after approval** (`A`) by Owner or Staff. `requires_review = true` output is invisible to clients until approved.
...
The AI orchestration layer (all eight logical sub-agents) has **zero direct write access to financial facts** — no INSERT/UPDATE/DELETE on financial profiles, credit profiles, income, debts, goals, readiness assessments, roadmaps, documents, referrals, or any other domain table.

Its complete write surface:

```text
ALLOWED WRITES (AI service DB role):
  ai_runs                    -- one row per orchestration run: agent, model, inputs hash,
                             --   status/outcome, response envelope, confidence, facts_used,
                             --   rules_used, reason_codes, requires_review,
...
- A recommendation becomes a real change only when an Owner or Staff member approves it, at which point an **application service** (running under the human's authorization context) performs the mutation, links it to the approving user and the `ai_runs` row, and emits an audit event.
- Any agent response with `prohibited_action_detected = true` is stored, flagged, and surfaced to staff; it is never auto-applied.
...
| 11 | `prohibited_action_detected` flagged by any agent | AI service | Surfaced to staff; never auto-applied. |
