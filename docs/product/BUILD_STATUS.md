...
- **Slice R — notification preferences** (founder order #9): `@aflo/notifications` gains channels (`in_app`/`email`/`sms`; push later) + per-type default routing + `NotificationPreferenceRecord` (append-only, latest-wins, granular per type×channel) + `resolveDelivery` (**enforced before send**: a disabled channel or an external channel without communication consent is withheld with a reason; in-app never needs external consent); store `setNotificationPreference` (audited, org/actor scoped) + `notificationPreferencesFor`, and `logNotification` now plans/logs **per resolved channel**; staff notification-preferences panel on client detail (per-channel toggles); seed opt-out (Grant appointment SMS). 7 kernel + 5 store tests + prefs e2e. _In PR (branch `claude/notification-preferences`)._
