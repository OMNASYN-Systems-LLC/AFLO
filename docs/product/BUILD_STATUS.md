...
- **Slice F — roadmap approval workflow**: allow-list state machine (`roadmap.v1.0.0`: Draft → Staff Review → Approved → Published, explicit returns/reopens/archival, registered + lockstep-tested); `Roadmap` entity linking existing milestones (one per client in V1); `RoadmapPublished` added to the event catalog (27 types); store transitions with approval/publication stamping, approval withdrawal on reopen, `RoadmapApproved`/`RoadmapPublished` outbox events, audited denials, tenant/actor isolation; client-detail roadmap card with status badge, provenance (authored/approved/published by whom), and only rule-legal actions. AI drafting deliberately deferred to F2 (workflow before agents). _In PR (branch `claude/roadmap-workflow`)._
- Slice G next: monthly action plan workflow (generation from approved roadmap + templates, completion tracking, TaskAssigned/TaskCompleted events).
