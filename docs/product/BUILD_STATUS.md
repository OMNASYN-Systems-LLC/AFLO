...
- **Slice O — round-up simulator**: `@aflo/rules` `roundup.v1.0.0` deterministic calculator (integer-cent round-up to a configurable boundary × multiplier, totals, 30-day projection — **SIMULATION ONLY**, never moves money or touches a real account); `SimulationSettings` + `VirtualTransaction` domain + seed (round-up amounts computed by the rule so seed can't drift); store `configureSimulation` + `addVirtualTransaction` (rule-computed round-up, validated, audited, isolated); client-detail simulator card with projected monthly/annual savings toward the primary goal + add-transaction form. Registered + lockstep-tested. 10 rules + 8 store tests + a simulator e2e. _In PR (branch `claude/roundup-simulator`)._
- Founder-action-gated (credentials/accounts, not code): Neon connection + repository swap (`DATABASE_URL`), outbox worker (needs the shared DB), Clerk activation (keys), Resend activation (keys), Stripe test-mode billing (keys + approved packages). Remaining credential-free modules: admin settings layer, goals workflow, engagement/retention analytics, observability instrumentation.
