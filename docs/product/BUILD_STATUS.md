...
- **Slice I — client portal**: `PortalView` projection + `MockPortalRepository` — published-only **by construction** (published roadmaps/reports only; stage from the latest recorded assessment *not* awaiting human review; no reason codes, review flags, drafts, pipeline internals, or staff notes are even representable); fails closed for leads/foreign-org/unknown ids; server-side demo client session (`getClientSession`, Marcus Bell persona — Clerk replaces with the same shape); `/portal` route with a separate client shell (no staff navigation reachable); sign-in shell's portal entry activated. Read-only this slice — client-initiated task completion arrives with real auth. _In PR (branch `claude/client-portal`)._
- Slice J / next queue: staff workflow completion (documents review states, appointment scheduling, notes) → Clerk auth (packages/auth) → Neon persistence (Drizzle) → outbox worker.
