...
- **Slice K — auth adapter boundary**: `@aflo/auth` activated from stub — provider-neutral `AuthSession` (staff/client kinds), `AuthProvider` interface, fail-closed `requireStaffSession`/`requireClientSession` guards, `DemoAuthProvider` personas; `apps/web` session resolution routed through the package (async `getStaffSession`/`getClientSession`); the Clerk-backed provider drops in behind the same guards/call sites when the founder issues credentials (ADR-0006, prepare-only). Deployment docs updated: **Vercel connected** (`aflo-web`, root `apps/web`, previews + production green). _In PR (branch `claude/auth-adapter`)._
- Next queue: Neon persistence (Drizzle behind unchanged repository interfaces) → outbox worker (apps/worker claim/complete/fail loop) → Resend notifications.
