...
- **Slice H — quarterly report workflow**: `report.v1.0.0` rules (Draft → Ready for review → Published; returns explicit; published terminal — never edited in place) + deterministic `quarterOf`/`quarterMonths`; canonical `LIFECYCLE_STAGE_LABELS` moved into the rules kernel (UI re-exports — report copy can never drift); `ProgressReportPublished` added to the event catalog (28 types); store `generateQuarterlyReport` — content derived only from **recorded** facts (latest readiness assessment required; quarter's action-plan stats; approved documents; published-roadmap milestone progress; focus = the assessment's proposed next action), one report per client-quarter, denials audited — and `transitionReport` with publication events; report card gains review-workflow actions and rules-gated generation. _In PR (branch `claude/quarterly-reports`)._
- Slice I next: client portal (published-only views: stage, published roadmap, monthly actions, published reports; consent-scoped).
