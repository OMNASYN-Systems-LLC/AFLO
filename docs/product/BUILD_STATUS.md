...
- **Slice E — readiness assessment workflow**: recorded `ReadinessAssessmentRecord` history (append-only; latest = standing) with stage, previous stage, rule version, reason codes, deterministic proposed-next-action per reason code (`REASON_CODE_NEXT_ACTIONS`, threshold-interpolated), and a deterministic staff-review gate (`review.v1.0.0` — regressions and multi-stage advances flagged, never AI); eligibility requires a completed intake; attempts without verified profiles are audited as blocked, never recorded; `ReadinessAssessed` outbox events; staff "Run assessment" on client detail with recorded-vs-current-facts display. _In PR (branch `claude/readiness-assessment`)._
- Slice F next: roadmap workflow (Drafted → Staff Review → Approved → Published; AI drafts language only, never structure-of-fact decisions).
