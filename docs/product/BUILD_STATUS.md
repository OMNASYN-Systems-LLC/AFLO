...
- **Slice J — staff workflow completion**: `document.v1.0.0` review-state rules (requested → uploaded → in_review → approved | needs_attention, re-entry via re-upload, approved terminal — replacement means a new request, never a silent downgrade); store `requestDocument` (`DocumentRequested`), `transitionDocument` (`DocumentUploaded` with a synthetic storage placeholder — real signed-URL storage stays a deferred requirement; `DocumentReviewed` on decisions), `scheduleAppointment` (validated future datetime, `AppointmentScheduled`), `addNote` (internal, audit-only — never portal-visible); client-detail documents card gains rule-legal review actions + request form, new Appointments card with upcoming list + scheduling, notes card gains entry form. _In PR (branch `claude/staff-workflows`)._
- Next queue: Clerk auth (packages/auth adapter) → Neon persistence (Drizzle behind unchanged interfaces) → outbox worker (apps/worker claim/complete/fail loop) → Resend notifications.
