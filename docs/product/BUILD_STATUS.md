...
- **Slice L — notifications kernel**: `@aflo/notifications` activated from stub — `notification.v1.0.0` consent-gated delivery: `hasActiveConsent` (append-only, latest-record-wins, fail-closed), typed template registry per `NotificationType` (each tied to a domain event; renderer fails closed on missing/blank vars), delivery-log allow-list state machine (queued → sent → delivered | bounced, failed → retry; suppressed/delivered/bounced terminal), `planNotification` (consent gate before any content is rendered), `NotificationProvider` boundary + `MockNotificationProvider`, versioned registry with lockstep tests. Mock delivery only — Resend activates on founder credentials (charter). 18 tests. Wiring to events/store + a staff communication log is the follow-up. _In PR (branch `claude/notifications-kernel`)._
- Next queue: notification wiring (emit on AppointmentScheduled/RoadmapPublished/ProgressReportPublished via the store + a staff comms log) → Neon persistence (Drizzle behind unchanged interfaces) → outbox worker.
