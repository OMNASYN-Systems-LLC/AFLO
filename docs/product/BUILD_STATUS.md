...
- **Slice M — notification wiring**: the kernel wired into the store — appointment scheduling, roadmap publication, report publication, document requests, and task assignment each plan a consent-gated communication (`planNotification` before any content is rendered), recorded in an append-only communication log (charter "communication history"). Consent records seeded per client (Harold Ngo revoked → his sends suppressed and **recorded with reason**, never silently dropped); staff Communications card on client detail shows sent/suppressed. Async provider dispatch with retries stays the worker slice. 7 store tests + a communications e2e. _In PR (branch `claude/notification-wiring`)._
- Next queue: Neon persistence (Drizzle schema-as-code + repositories behind unchanged interfaces) → outbox worker (apps/worker claim/complete/fail loop, dispatching queued notifications via the provider) → Stripe test-mode billing.
