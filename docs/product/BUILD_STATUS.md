...
- **Slice N — Drizzle schema foundation**: `@aflo/database` activated from stub (ADR-0005) — Postgres enums built **directly from the kernel constant arrays** (lifecycle/roadmap/report/document/action status, consent type) so DB and rules can't disagree, plus canonical domain enums lockstep-tested; core tables (identity `users`, tenancy `organizations`/`organization_members`, governance `consent_records`/`rule_versions`/`audit_events`/`outbox`, CRM `clients`/`intakes`) matching the **implemented** model (unified `clients` table w/ `client_kind` + configurable `pipeline_stage_id`, not separate leads/clients); PII columns encrypted `bytea` only; `drizzle.config` + **offline-generated migration** (9 tables, no connection needed); `DATABASE_SCHEMA.md` reconciled with an authoritative-source banner. 9 lockstep tests. _In PR (branch `claude/drizzle-schema`)._
- Next queue: workflow tables + Neon connection & repository swap (behind unchanged `@aflo/shared` interfaces, gated on `DATABASE_URL`) → outbox worker → Stripe test-mode billing.
