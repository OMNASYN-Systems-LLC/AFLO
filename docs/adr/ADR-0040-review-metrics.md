...
action completion, stage advancement, playbook effectiveness, staff
consistency, escalation volume — is the measurement layer of the moat: it is
...
- **`reviewMetricsFor(items, decisions, actions)`** → overall + per-artifact
  ...
  stage-advancement count, per-reviewer `StaffDecisionProfile`s (sorted,
...
- **9 new tests → 246 shared tests**: null-not-zero empty denominators,
