...
> **Source-of-truth order.** This brief is authoritative below `docs/product/PRODUCT_CHARTER.md`. Resolve conflicts using: (1) approved founder decisions — including `docs/product/FOUNDER_DIRECTIVE_2026-07-18.md` and `docs/product/FOUNDER_DIRECTIVE_2026-07-20.md`, (2) the Product Charter, (3) this file, (4) accepted ADRs, (5) architecture docs, (6) V1 scope, (7) partner-orchestration roadmap, (8) business-plan digest, (9) historical research. When this file and a higher source disagree, the higher source wins and this file is updated to match — never leave two active architectures.

> **Strategic differentiation (founder decision 2026-07-20).** ΛFLO is not a general-purpose financial chatbot. The moat is: Golden Key's real workflow + human review and professional accountability + versioned Professional Playbooks + longitudinal client context + source-labeled/verified facts + structured staff approval/rejection/override data + measurable outcomes + multi-tenant workflow infrastructure for small professional firms. Human-in-the-loop is a FIRST-CLASS workflow architecture: a Human Review Center with per-artifact review queues, unified states `draft → awaiting_review → approved → published` (terminals: `rejected`, `superseded`, `withdrawn`), full provenance on every reviewable artifact, structured review decisions (approved unchanged / approved with edits / rejected / escalated / deferred) with structured reason codes, and outcome tracking. **Never publish high-impact AI output directly to a client.** Playbooks are versioned tenant IP; the 10 initial Golden Key playbooks ship as editable drafts with a workflow-discovery queue — never invent the founder's actual process. Review-feedback data is not used for uncontrolled model training. Full directive: `docs/product/FOUNDER_DIRECTIVE_2026-07-20.md`.
